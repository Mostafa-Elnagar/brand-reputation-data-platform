...
resource "aws_sfn_state_machine" "reddit_listing_ingestion" {
  ...
  definition = templatefile("${path.module}/listing_ingestion_state_machine.asl.json", {
    ...
    silver_core_job_name    = data.terraform_remote_state.glue_enrichment.outputs.silver_core_job_name
    silver_sentiment_job_name = data.terraform_remote_state.glue_enrichment.outputs.silver_sentiment_job_name
  })
